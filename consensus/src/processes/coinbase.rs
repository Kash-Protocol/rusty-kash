...
impl CoinbaseManager {
    pub fn new(
        ...
        // In a 10 BPS network, the induced increase in total rewards is 51 KSH(see tests::calc_high_bps_total_rewards_delta())
...
/*
    This table was pre-calculated by calling `calcDeflationaryPeriodBlockSubsidyFloatCalc` (in kashd-go) for all months until reaching 0 subsidy.
...
mod tests {
    ...
    fn calc_high_bps_total_rewards_delta() {
        ...
        println!("Total rewards: {} sompi => {} KSH", total_rewards, total_rewards / SOMPI_PER_KASH);
        println!("Total high bps rewards: {} sompi => {} KSH", total_high_bps_rewards, total_high_bps_rewards / SOMPI_PER_KASH);
        println!("Delta: {} sompi => {} KSH", delta, delta / SOMPI_PER_KASH as i64);
    }
