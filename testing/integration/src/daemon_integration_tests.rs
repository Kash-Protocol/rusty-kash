...
use kash_rpc_core::{api::rpc::RpcApi, Notification};
...
async fn daemon_sanity_test() {
    ...
    let mut kashd1 = Daemon::new_random(total_fd_limit);
    let rpc_client1 = kashd1.start().await;

    let mut kashd2 = Daemon::new_random(total_fd_limit);
    let rpc_client2 = kashd2.start().await;
...
async fn daemon_mining_test() {
    ...
    let mut kashd1 = Daemon::new_random_with_args(args.clone(), total_fd_limit);
    let mut kashd2 = Daemon::new_random_with_args(args, total_fd_limit);
    let rpc_client1 = kashd1.start().await;
    let rpc_client2 = kashd2.start().await;

    rpc_client2.add_peer(format!("127.0.0.1:{}", kashd1.p2p_port).try_into().unwrap(), true).await.unwrap();
    ...
    for i in 0..10 {
        let template = rpc_client1
            .get_block_template(Address::new(kashd1.network.into(), kash_addresses::Version::PubKey, &[0; 32]), vec![])
            .await
            .unwrap();
    ...
    let vc = rpc_client2.get_virtual_chain_from_block(kash_consensus::params::SIMNET_GENESIS.hash, true).await.unwrap();
...
async fn daemon_cleaning_test() {
    ...
    {
        let total_fd_limit = 10;
        let mut kashd1 = Daemon::new_random_with_args(args, total_fd_limit);
        let dyn_consensus_manager = kashd1.core.find(ConsensusManager::IDENT).unwrap();
        let dyn_async_runtime = kashd1.core.find(AsyncRuntime::IDENT).unwrap();
        ...
        core = Arc::downgrade(&kashd1.core);
