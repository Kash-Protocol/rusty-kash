...
use kash_consensus_core::{constants::SOMPI_PER_KASH, network::NetworkType, tx::Transaction};
use kash_core::{debug, info};
use kash_notify::{
    listener::ListenerId,
    scope::{NewBlockTemplateScope, Scope},
};
use kash_rpc_core::{api::rpc::RpcApi, Notification, RpcError};
...
/// `cargo test --release --package kash-testing-integration --lib --features devnet-prealloc -- mempool_benchmarks::bench_bbt_latency --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
async fn bench_bbt_latency() {
    kash_core::panic::configure_panic();
    kash_core::log::try_init_logger("info,kash_core::time=debug,kash_mining::monitor=debug");
    ...
    */
    ...
    let prealloc_address =
        Address::new(NetworkType::Simnet.into(), kash_addresses::Version::PubKey, &prealloc_pk.x_only_public_key().0.serialize());
    ...
    let args = Args {
        ...
        prealloc_amount: 500 * SOMPI_PER_KASH,
    ...
    let pay_address =
        Address::new(network.network_type().into(), kash_addresses::Version::PubKey, &pk.x_only_public_key().0.serialize());
    ...
    let submit_block_pool = daemon
        .new_client_pool(SUBMIT_BLOCK_CLIENTS, 100, |c, block| async move {
            let _sw = kash_core::time::Stopwatch::<500>::with_threshold("sb");
            let response = c.submit_block(block, false).await.unwrap();
            assert_eq!(response.report, kash_rpc_core::SubmitBlockReport::Success);
            false
        })
        .await;
        ...
        .new_client_pool::<(usize, Arc<Transaction>), _, _>(SUBMIT_TX_CLIENTS, 100, |c, (i, tx)| async move {
            match c.submit_transaction(tx.as_ref().into(), false).await {
                ...
                Err(RpcError::General(msg)) if msg.contains("orphan") => {
                    kash_core::warn!("\n\n\n{msg}\n\n");
                    kash_core::warn!("Submitted {} transactions, exiting tx submit loop", i);
                    return true;
                }
                Err(e) => panic!("{e}"),
            }
        ...
        while let Ok(notification) = notification_rx.recv().await {
            match notification {
                Notification::NewBlockTemplate(_) => {
                    ...
                    // let _sw = kash_core::time::Stopwatch::<500>::with_threshold("bbt");
                    *current_template.lock() = cc.get_block_template(pac.clone(), vec![]).await.unwrap();
                }
                _ => panic!(),
            }
            if !exec.load(Ordering::Relaxed) {
                kash_core::warn!("Test is over, stopping miner receiver loop");
                break;
            }
        }
        kash_core::warn!("Miner receiver loop task exited");
    });
    ...
    let miner_loop_task = tokio::spawn(async move {
        for i in 0..BLOCK_COUNT {
            ...
            if !exec.load(Ordering::Relaxed) {
                kash_core::warn!("Test is over, stopping miner loop");
                break;
            }
        }
        ...
        kash_core::warn!("Miner loop task exited");
    });
    ...
    let tx_sender_task = tokio::spawn(async move {
        ...
        kash_core::warn!("Tx sender task, waiting for mempool to drain..");
        loop {
            if !exec.load(Ordering::Relaxed) {
                break;
            }
        ...
        exec.store(false, Ordering::Relaxed);
        kash_core::warn!("Tx sender task exited");
    });
