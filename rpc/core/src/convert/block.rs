...
use kash_consensus_core::block::{Block, MutableBlock};
    ...
    fn try_from(item: &RpcBlock) -> RpcResult<Self> {
        Ok(Self {
            ...
            transactions: Arc::new(
                item.transactions
                    .iter()
                    .map(kash_consensus_core::tx::Transaction::try_from)
                    .collect::<RpcResult<Vec<kash_consensus_core::tx::Transaction>>>()?,
            ),
        })
    }
}
