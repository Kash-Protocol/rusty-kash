use crate::protowire::{
    kashd_response::Payload, BlockAddedNotificationMessage, KashdResponse, NewBlockTemplateNotificationMessage, RpcNotifyCommand,
};
use crate::protowire::{
...
use kash_rpc_core::{Notification, RpcError, RpcHash};
use std::str::FromStr;
use std::sync::Arc;
...
from!(item: &kash_rpc_core::Notification, KashdResponse, { Self { id: 0, payload: Some(item.into()) } });

from!(item: &kash_rpc_core::Notification, Payload, {
    match item {
...
from!(item: &kash_rpc_core::BlockAddedNotification, BlockAddedNotificationMessage, { Self { block: Some((&*item.block).into()) } });

from!(&kash_rpc_core::NewBlockTemplateNotification, NewBlockTemplateNotificationMessage);

from!(item: &kash_rpc_core::VirtualChainChangedNotification, VirtualChainChangedNotificationMessage, {
    Self {
...
from!(item: &kash_rpc_core::FinalityConflictNotification, FinalityConflictNotificationMessage, {
    Self { violating_block_hash: item.violating_block_hash.to_string() }
});

from!(item: &kash_rpc_core::FinalityConflictResolvedNotification, FinalityConflictResolvedNotificationMessage, {
    Self { finality_block_hash: item.finality_block_hash.to_string() }
});

from!(item: &kash_rpc_core::UtxosChangedNotification, UtxosChangedNotificationMessage, {
    Self {
...
from!(item: &kash_rpc_core::SinkBlueScoreChangedNotification, SinkBlueScoreChangedNotificationMessage, {
    Self { sink_blue_score: item.sink_blue_score }
});

from!(item: &kash_rpc_core::VirtualDaaScoreChangedNotification, VirtualDaaScoreChangedNotificationMessage, {
    Self { virtual_daa_score: item.virtual_daa_score }
});

from!(&kash_rpc_core::PruningPointUtxoSetOverrideNotification, PruningPointUtxoSetOverrideNotificationMessage);
...
try_from!(item: &KashdResponse, kash_rpc_core::Notification, {
    item.payload
        .as_ref()
        .ok_or_else(|| RpcError::MissingRpcFieldError("KashdResponse".to_string(), "payload".to_string()))?
        .try_into()?
});

try_from!(item: &Payload, kash_rpc_core::Notification, {
    match item {
...
try_from!(item: &BlockAddedNotificationMessage, kash_rpc_core::BlockAddedNotification, {
    Self {
        block: Arc::new(
            item.block
                .as_ref()
...
try_from!(&NewBlockTemplateNotificationMessage, kash_rpc_core::NewBlockTemplateNotification);

try_from!(item: &VirtualChainChangedNotificationMessage, kash_rpc_core::VirtualChainChangedNotification, {
    Self {
...
try_from!(item: &FinalityConflictNotificationMessage, kash_rpc_core::FinalityConflictNotification, {
    Self { violating_block_hash: RpcHash::from_str(&item.violating_block_hash)? }
});

try_from!(item: &FinalityConflictResolvedNotificationMessage, kash_rpc_core::FinalityConflictResolvedNotification, {
    Self { finality_block_hash: RpcHash::from_str(&item.finality_block_hash)? }
});

try_from!(item: &UtxosChangedNotificationMessage, kash_rpc_core::UtxosChangedNotification, {
    Self {
...
try_from!(item: &SinkBlueScoreChangedNotificationMessage, kash_rpc_core::SinkBlueScoreChangedNotification, {
    Self { sink_blue_score: item.sink_blue_score }
});

try_from!(item: &VirtualDaaScoreChangedNotificationMessage, kash_rpc_core::VirtualDaaScoreChangedNotification, {
    Self { virtual_daa_score: item.virtual_daa_score }
});

try_from!(&PruningPointUtxoSetOverrideNotificationMessage, kash_rpc_core::PruningPointUtxoSetOverrideNotification);
