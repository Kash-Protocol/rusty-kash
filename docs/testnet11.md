...
*Testnet 11* is the network where we will launch the first public 10BPS Kash experiment. The goal of this document is to provide a quick guide for anyone who wants to participate.
...
1. *kashd* - the Kash client
2.  *kashminer* - the Kash CPU miner
...
1. Download and extract the [rusty-kash binaries](https://github.com/Kash-Protocol/rusty-kash/releases). Alternatively, you can compile it from source yourself by following the instructions [here](https://github.com/Kash-Protocol/rusty-kash/blob/master/README.md). The rest of the instructions are written assuming the former option. If you choose to locally compile the code, replace any command of the form ``<program> <arguments>`` with ``cargo run --bin <program> --release -- <arguments>`` (see example in the next item). All actions described below should be performed on a command line window where you navigated to the directory into which the binaries were extracted.
2. Start the ``kashd`` client with ``utxoindex`` enabled:

```
kashd --testnet --netsuffix=11 --utxoindex
```
...
```
cargo run --bin kashd --release -- --testnet --netsuffix=11 --utxoindex
```
...
2. The output will provide you with a private key (that looks like a bunch of gibberish) and a public address (that looks like "kashtest:" followed by a bunch of gibberish). For example, the output could look like this:
     ```
     2023-06-25 18:00:58.677+00:00 [INFO ] Connected to RPC
     2023-06-25 18:00:58.677+00:00 [INFO ] Generated private key aa1c554386218eb28c4bsf6a02e5943799cf951dac7301324d88dec2d0119fce and address kashtest:qzlpwt49f0useql6w0tzpnf8k2symdv5tu2x2pe9r9nvngw8mvx57q0tt9lr5. Send some funds to this address and rerun rothschild with `--private-key aa1c554386218eb28c4bsf6a02e5943799cf951dac7301324d88dec2d0119fce`
      ```
     Here, the private key is ```aa1c554386218eb28c4bsf6a02e5943799cf951dac7301324d88dec2d0119fce``` and the address is ```kashtest:qzlpwt49f0useql6w0tzpnf8k2symdv5tu2x2pe9r9nvngw8mvx57q0tt9lr5```
...
Like kashd, the Rothschild window should remain open and undisturbed.

For mining, grab `kash-miner` from within the latest [Release](https://github.com/elichai/kash-miner/releases) and run it with the following flags (**this is currently the only miner that supports testnet-11**):
    ```
    kash-miner --testnet --mining-address <address> -p 16210 -t 1
    ```
...
Like the Kashd and Rothschild windows, the miner window should also be left undisturbed, and closing it will stop the mining.
