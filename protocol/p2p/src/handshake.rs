...
use crate::pb::{kashd_message::Payload, ReadyMessage, VerackMessage, VersionMessage};
use crate::{common::ProtocolError, dequeue_with_timeout, make_message};
use crate::{IncomingRoute, KashdMessagePayloadType, Router};
use kash_core::debug;

/// Implements the Kash peer-to-peer handshake protocol
pub struct KashdHandshake<'a> {
    router: &'a Router,
...
impl<'a> KashdHandshake<'a> {
    /// Builds the handshake object and subscribes to handshake messages
    pub fn new(router: &'a Router) -> Self {
        Self {
            router,
            version_receiver: router.subscribe(vec![KashdMessagePayloadType::Version]),
            verack_receiver: router.subscribe(vec![KashdMessagePayloadType::Verack]),
            ready_receiver: router.subscribe(vec![KashdMessagePayloadType::Ready]),
        }
    }
