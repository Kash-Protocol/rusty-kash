...
use kash_consensus_core::{tx::ScriptPublicKeys, utxo::utxo_diff::UtxoDiff, BlockHashSet};
use kash_consensusmanager::{ConsensusManager, ConsensusResetHandler};
use kash_core::{info, trace};
use kash_database::prelude::{StoreError, StoreResult, DB};
use kash_hashes::Hash;
use kash_index_core::indexed_utxos::BalanceByScriptPublicKey;
use kash_utils::arc::ArcExtensions;
...
const RESYNC_CHUNK_SIZE: usize = 2048; //Increased from 1k (used in go-kashd), for quicker resets, while still having a low memory footprint.
...
impl UtxoIndexApi for UtxoIndex {
    ...
    fn get_all_outpoints(&self) -> StoreResult<std::collections::HashSet<kash_consensus_core::tx::TransactionOutpoint>> {
