...
use kash_bip32::secp256k1::XOnlyPublicKey;
use kash_wallet_core::message::{sign_message, verify_message, PersonalMessage};
...
impl Message {
    async fn main(self: Arc<Self>, ctx: Arc<KashCli>, argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
        ...
        match argv.first().unwrap().as_str() {
            ...
            "verify" => {
                ...
                self.verify(ctx, kash_address, signature, message).await?;
            }
            v => {
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<KashCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ("sign <kash_address>", "Sign a message with the private key that matches the given address. Prompts for message."),
                (
                    "verify <kash_address> <signature>",
                    "Verify the signature against the message and kash_address. Prompts for message.",
                ),
            ],
            None,
        )?;
    ...
    async fn sign(self: Arc<Self>, ctx: Arc<KashCli>, kash_address: &str, message: &str) -> Result<()> {
        let kash_address = Address::try_from(kash_address)?;
        if kash_address.version != Version::PubKey {
        ...
        let privkey = self.get_address_private_key(&ctx, kash_address).await?;
    ...
    async fn verify(self: Arc<Self>, ctx: Arc<KashCli>, kash_address: &str, signature: &str, message: &str) -> Result<()> {
        let kash_address = Address::try_from(kash_address)?;
        if kash_address.version != Version::PubKey {
            return Err(Error::custom("Address not supported for message signing. Only supports PubKey addresses"));
        }

        let pubkey = XOnlyPublicKey::from_slice(&kash_address.payload[0..32]).unwrap();
    ...
    async fn get_address_private_key(self: Arc<Self>, ctx: &Arc<KashCli>, kash_address: Address) -> Result<[u8; 32]> {
        ...
        match account.account_kind() {
            AccountKind::Bip32 => {
                ...
                let (receive, change) = account.derivation().addresses_indexes(&[&kash_address])?;
