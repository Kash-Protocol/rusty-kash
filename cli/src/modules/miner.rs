use crate::imports::*;
use kash_daemon::{locate_binaries, CpuMinerConfig};
...
impl DefaultSettings for MinerSettings {
    async fn defaults() -> Vec<(Self, Value)> {
        ...
        if let Ok(binaries) = locate_binaries(&root, "kash-cpu-miner").await {
...
impl Miner {
    ...
    async fn create_config(&self, ctx: &Arc<KashCli>) -> Result<CpuMinerConfig> {
    ...
    async fn select(self: Arc<Self>, ctx: Arc<KashCli>) -> Result<()> {
        let root = nw_sys::app::folder();

        let binaries = kash_daemon::locate_binaries(root.as_str(), "kash-cpu-miner").await?;

        if binaries.is_empty() {
            tprintln!(ctx, "No kash-cpu-miner binaries found");
        } else {
            let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
            if let Some(selection) = ctx.term().select("Please select kash-cpu-miner binary", &binaries).await? {
