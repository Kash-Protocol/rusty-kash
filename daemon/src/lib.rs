...
pub use kashd::{Kashd, KashdConfig, KashdCtl};
...
pub static LOCATIONS: &[&str] = &[
    ...
    "../target/release",
    "../target/debug",
    "../../kash-cpu-miner/target/debug",
    "../../kash-cpu-miner/target/release",
    "bin/windows-x64",
    "bin/linux-ia32",
    "bin/linux-x64",
    "bin/linux-arm64",
...
#[derive(Default)]
pub struct Daemons {
    pub kashd: Option<Arc<dyn KashdCtl + Send + Sync + 'static>>,
    // pub kashd_automute : Arc<
    pub cpu_miner: Option<Arc<dyn CpuMinerCtl + Send + Sync + 'static>>,
}

impl Daemons {
    pub fn new() -> Self {
        Self { kashd: None, cpu_miner: None }
    }

    pub fn with_kashd(mut self, kashd: Arc<dyn KashdCtl + Send + Sync + 'static>) -> Self {
        self.kashd = Some(kashd);
        self
    }
    ...
    pub fn kashd(&self) -> Arc<dyn KashdCtl + Send + Sync + 'static> {
        self.kashd.as_ref().expect("accessing Daemons::kashd while kashd option is None").clone()
    }

    pub fn try_kashd(&self) -> Option<Arc<dyn KashdCtl + Send + Sync + 'static>> {
        self.kashd.clone()
    }
