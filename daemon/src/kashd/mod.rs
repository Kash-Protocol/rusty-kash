...
impl TryFrom<KashdConfig> for Vec<String> {
    type Error = Error;
    fn try_from(args: KashdConfig) -> Result<Vec<String>> {
        let mut argv = Vec::new();

        if args.path.is_none() {
            return Err(Error::Custom("no kashd path is specified".to_string()));
        }
...
impl Kashd {
    pub fn new(args: KashdConfig) -> Self {
        Self {
    ...
    pub fn configure(&self, config: KashdConfig) -> Result<()> {
    ...
    pub fn start(&self) -> Result<()> {
        ...
        if let Some(process) = process {
            if process.is_running() {
                return Err(Error::Custom("Kash node is already running.".to_string()));
            }
        }
        ...
        // let options = KashdOptions::new(path,network)?;
    ...
    pub async fn version(&self) -> Result<String> {
        ...
        if let Some(path) = path {
            ...
            Ok("Kashd binary is not configured. Please use 'node select' command.".to_string())
        }
    }
}
...
pub trait KashdCtl {
    ...
    async fn configure(&self, config: KashdConfig) -> Result<()>;
