//!
//! API trait for interfacing with the Kash wallet subsystem.
//!
...
pub trait WalletApi: Send + Sync + AnySync {
    ...
    /// If the `filename` is `None`, the wallet opens the default wallet named `kash`.
    ///
    ...
    /// applications such as KDX and kashnet.io web wallet using a deprecated derivation path
