...
use kash_consensus_core::network::{NetworkId, NetworkType};
    ...
    F: FnOnce(NetworkType) -> Address + Clone,
{
    let outputs = outputs
        ...
        .map(|(address, amount)| {
            ...
            (address.clone()(network_id.into()), sompi.0, KSH)
        })
        .collect::<Vec<_>>();
    make_generator(
        network_id,
    ...
    F: FnOnce(NetworkType) -> Address,
{
    ...
    let utxo_entries: Vec<UtxoEntryReference> = values.into_iter().map(kash_to_sompi).map(UtxoEntryReference::simulated).collect();
...
fn test_generator_sweep_two_utxos() -> Result<()> {
    ...
    .fetch(&Expected {
        is_final: true,
        input_count: 2,
        aggregate_input_value: Kash(20.0),
        output_count: 1,
...
fn test_generator_compound_200k_10kas_transactions() -> Result<()> {
    generator(test_network_id(), &[10.0; 200_000], &[], Fees::sender(Kash(5.0)), [(output_address, Kash(190_000.0))].as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_compound_100k_random_transactions() -> Result<()> {
    ...
    let outputs = [(output_address, Kash(total - 10.0))];
    generator(test_network_id(), &inputs, &[], Fees::sender(Kash(5.0)), outputs.as_slice()).unwrap().harness().validate().finalize();
...
fn test_generator_random_outputs() -> Result<()> {
    ...
    let outputs: Vec<_> = outputs.into_iter().map(|v| (output_address, Kash(v))).collect();

    generator(test_network_id(), &[total + 100.0], &[], Fees::sender(Kash(5.0)), outputs.as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_dust_1_1() -> Result<()> {
    generator(
        ...
        &[],
        Fees::sender(Kash(5.0)),
        [(output_address, Kash(1.0)), (output_address, Kash(1.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
        is_final: true,
        input_count: 4,
        aggregate_input_value: Kash(40.0),
        output_count: 3,
        priority_fees: FeesExpected::sender(Kash(5.0)),
    })
    .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_exclude_success() -> Result<()> {
    // generator(test_network_id(), &[10.0; 100], &[], Fees::sender(Kash(5.0)), [(output_address, Kash(990.0))].as_slice())
    generator(test_network_id(), &[10.0; 100], &[], Fees::sender(Kash(0.0)), [(output_address, Kash(990.0))].as_slice())
        .unwrap()
        .harness()
        .fetch(&Expected {
            is_final: false,
            input_count: 88,
            aggregate_input_value: Kash(880.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
            is_final: false,
            input_count: 12,
            aggregate_input_value: Kash(120.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
            ...
            output_count: 2,
            // priority_fees: FeesExpected::sender(Kash(5.0)),
            priority_fees: FeesExpected::sender(Kash(0.0)),
        })
        .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_include_success() -> Result<()> {
    generator(
        ...
        &[1.0; 100],
        &[],
        Fees::receiver(Kash(5.0)),
        // [(output_address, Kash(100.0))].as_slice(),
        [(output_address, Kash(100.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
        is_final: false,
        input_count: 88,
        aggregate_input_value: Kash(88.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
        is_final: false,
        input_count: 12,
        aggregate_input_value: Kash(12.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
        ...
        output_count: 1,
        priority_fees: FeesExpected::receiver(Kash(5.0)),
    })
    .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_exclude_insufficient_funds() -> Result<()> {
    generator(test_network_id(), &[10.0; 100], &[], Fees::sender(Kash(5.0)), [(output_address, Kash(1000.0))].as_slice())
        .unwrap()
        .harness()
        .fetch(&Expected {
            is_final: false,
...
fn test_generator_inputs_903_outputs_2_fees_exclude() -> Result<()> {
    generator(test_network_id(), &[10.0; 1_000], &[], Fees::sender(Kash(5.0)), [(output_address, Kash(9_000.0))].as_slice())
        .unwrap()
        .harness()
        .drain(
            10,
        ...
        .fetch(&Expected {
            is_final: false,
            input_count: 21,
            aggregate_input_value: Kash(210.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
