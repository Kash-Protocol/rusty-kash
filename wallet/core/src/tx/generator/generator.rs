...
use kash_consensus_core::tx::{Transaction, TransactionAction, TransactionInput, TransactionOutpoint, TransactionOutput};
...
fn script_engine_validator(generator : &Generator, tx: &Transaction, utxo_entry_references: &Vec<UtxoEntryReference>, addresses : &HashSet<Address>) -> Result<()> {
    ...
    use kash_consensus_core::tx::{PopulatedTransaction,VerifiableTransaction,MutableTransaction};
    use kash_consensus_core::hashing::sighash::SigHashReusedValues;
    use kash_consensus_core::errors::tx::TxRuleError;
    use kash_txscript::{SigCacheKey,TxScriptEngine,caches::Cache};
