...
struct GeneratorSettings {
    ...
    pub final_transaction_action: TransactionAction,
...
impl TryFrom<GeneratorSettingsObject> for GeneratorSettings {
    ...
    fn try_from(args: GeneratorSettingsObject) -> std::result::Result<Self, Self::Error> {
        ...
        let final_transaction_action = match TransactionAction::try_from(args.get_value("transactionAction")?) {
            Ok(transaction_action) => transaction_action,
            Err(_err) => {
                return Err(Error::custom("Invalid transactionAction value"));
            }
        };
